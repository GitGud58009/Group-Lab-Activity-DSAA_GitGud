...
def BILL():
    ...
    def add_bill():
        ...
        total_due_label.config(text=f"₱{total_due:.2f}")
    ...
    def check_overdue_bills():
        ...
        for item in bill_tree.get_children():
            ...
            if current_time > due_date and item not in bill_tree.selection():
                ...
                bill_tree.tag_configure('overdue', foreground='red')
            else:
    ...
    date_label.place(relx=0.63, rely=0.25, anchor="w")

    time_label = Label(frame_BILL, font=("Helvetica", 18))
    time_label.place(relx=0.63, rely=0.145, anchor="w")
...
def TDL():
    ...
    class TodoListGUI:
        def __init__(self, master):
            ...
            deletebutton = PhotoImage(file=r"delete3.png")
            btndelete = Button(frame_TDL, image=deletebutton, command=self.remove_task, bd=0, compound=CENTER, padx=0, pady=0)
...
def ABT():
    ...
    pil5_image = Image.open(r"about3.png")
    aboutimage = ImageTk.PhotoImage(pil5_image)

    canvas = Canvas(frame_ABT, width=900, height=600)
    canvas.pack(fill="both", expand=True)
    canvas.create_image(0, 0, anchor="nw", image=aboutimage)

    homebutton = PhotoImage(file=r"rsz_homebutton4.png")
    btnhome = Button(frame_ABT, image=homebutton, command=Home, bd=0, compound=CENTER, padx=0, pady=0)
    btnhome.place(x=845, y=565)
